/**
 * RisuBard NodeOnly — English help texts (`language.help`).
 *
...
export const helpEn = {
        ...
        loreSelective: "With Multiple keys enabled, both an activation key and a secondary key must match to activate the lore.",
        ...
        chatPageSize: "Number of messages rendered on one chat page. Moving between pages removes the previous page DOM so browser memory does not keep growing with long chats.",
        ...
        antiClaudeOverload:
            "If Claude overload happens, RisuBard would try to prevent it by continuing with same prompt, making it less likely to happen. works only for streamed responses. this could not work for non-official api endpoints.",
        triggerScript:
        ...
        defaultVariables:
            "Here you can define your own default variables. use `<variable name>=<variable value>` format, seperated by newline. for example, `name=RisuBard`, which then can be used with trigger scripts and variables CBS like `{{getvar::A}}`, `{{setvar::A::B}}` or `{{? $A + 1}}`. if prompt template's default variable and character's default variable has same name, character's default variable will be used.",
        lowLevelAccess:
        ...
        nodeOnlyStandardChatWidth: "Maximum chat card width in the RisuBard Standard theme.",
        ...
        novellistKey:
            "API key for NovelList (a Korean AI writing service). The model lineup and access rules change frequently — check NovelList's own guidance before use.",
        mancerKey:
        ...
        ollamaURL:
            "URL of your local or remote Ollama server (e.g. `http://localhost:11434`). Pairs well with RisuBard's local-network mode for stable access to private LAN LLMs.",
        ollamaModel:
        ...
        memType:
            "Long-term memory mode.\n\n- **None**: disabled (chat is sent as-is up to the max context limit)\n- **HypaV3**: RisuBard's long-term memory that auto-summarizes and retrieves older chat to inject into the context. Slightly more cost / latency, but consistency in long chats improves a lot.",
        hypaV3SummaryModel:
        ...
        bootBackupReminder:
            "When enabled, RisuBard prompts you on every boot whether to create a server backup right away. Useful as a lightweight safety net before opening the app each session. Confirming runs a full server backup (the loading screen waits while it finishes); skipping continues straight to the app.",
}
