/**
 * 리스바드 NodeOnly — Korean help texts (`language.help`).
 *
...
export const helpKo = {
        ...
        "loreSelective": "멀티플 키를 켜면 활성화 키와 보조 키가 모두 일치해야 로어가 활성화됩니다.",
        ...
        "chatPageSize": "채팅 화면 한 페이지에 렌더링할 메시지 수입니다. 페이지를 이동하면 이전 페이지의 DOM을 제거해 긴 채팅에서도 브라우저 메모리 사용량이 계속 증가하지 않도록 제한합니다.",
        ...
        "antiClaudeOverload": "Claude 과부하가 발생하면, 리스바드는 동일한 프롬프트로 계속 진행하여 과부하 발생 가능성을 줄이려고 시도합니다. 스트리밍 응답에서만 작동합니다. 공식 API 엔드포인트가 아닌 경우 작동하지 않을 수 있습니다.",
        ...
        "defaultVariables": "여기에서는 기본 변수를 정의할 수 있습니다. `<변수 이름>=<변수 값>` 형식으로 작성하고 개행으로 구분합니다. 예를 들어, `name=리스바드`는 트리거 스크립트 및 변수 CBS와 함께 `{{getvar::A}}`, `{{setvar::A::B}}` 또는 `{{? $A + 1}}`과 같이 사용할 수 있습니다. 프롬프트 템플릿의 기본 변수와 캐릭터의 기본 변수가 동일한 이름을 가진 경우 캐릭터의 기본 변수가 사용됩니다.",
        ...
        "nodeOnlyStandardChatWidth": "리스바드 Standard 테마에서 메시지 카드의 최대 가로폭입니다.",
        ...
        "risuBardArcaChatImageWidthPercent": "추출된 챗 프레임 너비를 기준으로 이미지가 차지할 비율입니다. 모든 캐릭터와 챗에 공통 적용됩니다.",
        "risuBardArcaChatFontSizePx": "추출된 아카라이브 챗 프레임의 기본 글자 크기입니다. 모든 캐릭터와 챗에 공통 적용됩니다.",
        "risuBardArcaChatParagraphSpacingPercent": "추출된 문단과 이미지 블록 사이의 세로 간격입니다. 글꼴 높이를 기준으로 하며 100%는 1em입니다. 모든 캐릭터와 챗에 공통 적용됩니다.",
        "risuBardArcaChatShowTitleImage": "아카라이브 챗 맨 위에 해당 봇의 타이틀 이미지를 넣을지 정합니다. 모든 캐릭터와 챗에 공통 적용됩니다.",
        "risuBardArcaChatTitleImageStyle": "큰 타원형, 정사각형 크롭, 또는 작은 썸네일을 타이틀 옆에 배치하는 형식 중 하나를 고릅니다.",
        ...
        "risuBardWikiWritingLanguage": "사건 분석·요약과 정본 전체 본문을 작성할 언어입니다. UI·대화 언어와 독립적이며 현재 챗 설정으로 덮어쓸 수 있습니다. 사용자 문체·Wiki Guide의 언어 지시보다 우선합니다. 기존 제목·링크·고유명사·근거 인용은 보존합니다. 기존 문서는 자동 번역되지 않습니다. 전체 위키를 바꾸려면 언어 선택 후 리부트하세요. 리부트는 시작 언어를 유지하며 다른 언어의 기존 사건에 추가 분석을 덧붙이지 않습니다.",
        "risuBardCanonicalCustomStyle": "문체와 표현 형식만 지정할 수 있습니다. 사실 선택, 근거, 구조와 안전 규칙은 이 지시문보다 우선합니다. 최대 1,000자입니다.",
        "risuBardAnalysisTokenLimit": "각 메모리 분석·리부트·정본 생성 요청의 입력 및 출력 토큰 상한입니다. 긴 입력은 요청 전에 이 예산에 맞추며 모델 출력에도 같은 값이 적용됩니다. 기본값은 8,192이며 앱의 고정 최대값은 없지만 사용하는 모델의 컨텍스트·출력 한도 안에서 설정해야 합니다.",
        ...
        "risuBardArcPlotterMaxArcs": "플롯의 ‘아크 개요’에 유지할 시간순 아크 글머리표 상한입니다. 넘치면 오래된 인접 아크를 합칩니다.",
        "risuBardArcPlotterMaxTurningPoints": "이야기 방향을 바꾼 주요 전환점 글머리표 상한입니다.",
        ...
        "risuBardResponseExcludeUsers": "켜면 과거 사용자 메시지를 답변 참조 범위에서 제외합니다. 현재 사용자 요청은 항상 포함합니다.",
        ...
        "novellistKey": "NovelList(한국 AI 글쓰기 서비스) API 키. 비공식 모델이 많고 서비스가 자주 바뀌므로 사용 전 NovelList 측 안내를 확인하세요.",
        ...
        "ollamaURL": "로컬 또는 원격 Ollama 서버의 URL (예: `http://localhost:11434`). 리스바드의 \"로컬 네트워크 모드\"와 함께 쓰면 사설망 LLM도 안정적으로 호출됩니다.",
        ...
        "memType": "장기 메모리 방식 선택.\n\n- **None**: 사용 안 함 (max context 한도까지 채팅 그대로 사용)\n- **HypaV3**: 오래된 채팅을 자동으로 요약·검색해 컨텍스트에 주입하는 리스바드의 장기 메모리. 비용/지연이 약간 늘지만 긴 채팅에서 일관성이 크게 향상됩니다.",
