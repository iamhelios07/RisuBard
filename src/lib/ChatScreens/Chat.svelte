<script lang="ts">
    ...
    let memoryConfirming = $state(false)
        ...
        if(window.navigator.clipboard.write){
            try {
                ...
                for(const img of imgs){
                    img.setAttribute('alt', 'from RisuBard')
...
<div style="text-align: center; margin-top: 1rem; padding-top: 0.75rem; border-top: 1px solid ${root.style.getPropertyValue('--risu-theme-darkborderc')};">
    <span style="font-size: 0.75rem; color: ${root.style.getPropertyValue('--risu-theme-textcolor2')}; opacity: 0.7;">From RisuBard</span>
</div>
</div>
</div>`
...
{#snippet minorIconButtonsBody(showNames:boolean)}
    ...
    {#if role !== 'user'}
        <button
            ...
            onclick={async () => {
                ...
                try {
                    const confirmed = await onConfirmMemory(current.chatId)
                    if (confirmed) {
                        memoryConfirmed = true
                        statusMessage = language.risubardMemoryConfirmed
                    }
                    else {
                        statusMessage = language.risubardMemoryAlreadyConfirmed
                    }
                }
                catch {
                    statusMessage = language.risubardMemoryConfirmFailed
                }
                finally {
        ...
        >
            ...
            {#if showNames}
                <span class="ml-1">
                    {memoryConfirmed
                        ? language.risubardMemoryConfirmed
    ...
    <button class="flex items-center hover:text-primary transition-colors" onclick={async () => {
        ...
        if(DBState.db.createFolderOnBranch && !currentChat.folderId){
            ...
            currentCharacter.chatFolders.unshift({
                id: createdFolderId,
                name: `Branches of ${currentChat.name}`,
                folded: false,
            })
            currentChat.folderId = createdFolderId
            newChat.folderId = createdFolderId
        }
        ...
        try {
            ...
            currentChat.folderId = originalFolderId
            if(createdFolderId){
                currentCharacter.chatFolders = currentCharacter.chatFolders
            ...
            notifyError(
                `Chat branch save failed: ${error instanceof Error
                    ? error.message
