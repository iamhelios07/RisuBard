...
function normalizeLorebookId(value: unknown): string | undefined {
...
export function convertCharbook(arg:{
    ...
    for(const book of charbook.entries){
        ...
        lorebook.push({
            ...
            bookVersion: extensions.risu_bookVersion,
            id: normalizeLorebookId(extensions.risu_id ?? book.id),
            folder: book.folder
        })
    }
...
export function createBaseV2(char:character) {
    ...
    for(const lore of exportGlobalLore){
        ...
        if (lore.bookVersion !== undefined) {
            ;(ext as Record<string, unknown>).risu_bookVersion = lore.bookVersion
        }
        const loreId = normalizeLorebookId(lore.id)
        const numericLoreId = Number(loreId)
        const cardId = loreId !== undefined
            && Number.isSafeInteger(numericLoreId)
            && String(numericLoreId) === loreId.trim()
            ? numericLoreId
            : undefined
        if (loreId !== undefined && cardId === undefined) {
            ;(ext as Record<string, unknown>).risu_id = loreId
        }
        else if (cardId !== undefined) {
            delete (ext as Record<string, unknown>).risu_id
        }
        ...
        charBook.push({
            ...
            extensions: ext,
            enabled: isLorebookEntryEnabled(lore),
