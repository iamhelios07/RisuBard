...
describe('sendGoogleChatRequest (non-stream)', () => {
    ...
    test('makes caller structured-output limits authoritative on Gemini', async () => {
        const preset = makePreset({
    ...
    test.each([
        ...
        const { fetchImpl, calls } = captureFetch(
            jsonResponse({
                candidates: [{
                    content: { parts: [{ text: '{"action":"create"}' }] },
                }],
            }),
        )
        ...
        expect(calls[0].body.generationConfig)
            .not.toHaveProperty('responseSchema')
    })

    test('limits Gemini 2.5 thinking for structured internal calls', async () => {
        const preset = makePreset({
